...
const AllBookingsScreen = () => {
  ...
  const fetchBookings = async () => {
    try {
      const response = await adminAPI.getAdminBookings(0, 50);
      ...
      <View style={styles.cardContent}>
        <View style={styles.infoRow}>
          <View style={[styles.iconContainer, { backgroundColor: theme.colors.surface }]}>
            <ProfileIcon size={16} color={theme.colors.primary} />
          </View>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={[styles.infoText, { color: theme.colors.textSecondary }]}>
            {format(new Date(item.bookingDate || item.createdAt || new Date()), 'dd MMM yyyy')}
          </Text>
        </View>
        <View style={styles.infoRow}>
          <View style={[styles.iconContainer, { backgroundColor: theme.colors.surface }]}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={[styles.priceText, { color: theme.colors.text }]}>₹{item.amountBreakdown.totalAmount}</Text>
        </View>

        {item.childBookings && item.childBookings.length > 0 && (
          <View style={[styles.slotsContainer, { backgroundColor: theme.colors.surface }]}>
            ...
            {item.childBookings.map((ref, index) => (
