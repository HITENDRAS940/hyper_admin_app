...
export const authAPI = {
  ...
  requestEmailOtp: (email: string) =>
    api.post('/auth/request-email-otp', { email }).then((res) => res.data),
  verifyEmailOtp: (email: string, otp: string) =>
    api.post('/auth/verify-email-otp', { email, otp }).then((res) => res.data),
};
...
export const adminAPI = {
  ...
  getServiceBookings: async (serviceId: number, date: string): Promise<any> => {
    const res = await api.get(`/admin/service/${serviceId}/bookings`, {
      params: { date },
    });
    return res.data;
  },
  ...
  updateBookingStatus: async (
    ...
    const res = await api.post(`/admin/booking/${bookingId}/status`, {
      status,
      reason,
    });
    return res.data;
  ...
  updateAttendanceStatus: async (
    ...
    const res = await api.post(`/admin/booking/${bookingId}/attendance`, {
      attendanceStatus,
    });
    return res.data;
  },
  ...
  getSlotStatus: async (serviceId: string, date: string): Promise<any> => {
    const res = await api.get(`/admin/service/${serviceId}/slot-status`, {
      params: { date },
    });
    return res.data;
  },
  updateSlotPrice: async (
    serviceId: string,
    slotId: number,
    price: number,
  ): Promise<any> => {
    const res = await api.post(
      `/admin/service/${serviceId}/slot/${slotId}/price`,
      null,
      { params: { price } },
    );
    return res.data;
  ...
  enableSlot: async (serviceId: string, slotId: number): Promise<any> => {
    const res = await api.post(
      `/admin/service/${serviceId}/slot/${slotId}/enable`,
    );
    return res.data;
  },
  disableSlot: async (serviceId: string, slotId: number): Promise<any> => {
    const res = await api.post(
      `/admin/service/${serviceId}/slot/${slotId}/disable`,
    );
    return res.data;
  },
  ...
  setServiceApproval: async (
    ...
    const res = await api.patch(`/admin/service/${serviceId}/approval`, {
      status,
    });
    return res.data;
  },
